...
class Cube extends Sensor {
	...
	fireEvent(number, initial=false, gesture=null) {
		if (gesture !== 7 && gesture !== 8) {
			this.side = parseInt(number.toString().charAt(0)) // first digit
			if (initial) return
		}
		if (gesture == 0) {
			this.log('wake_up')
